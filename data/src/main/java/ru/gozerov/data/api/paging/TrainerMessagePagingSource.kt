...
class TrainerMessagePagingSource @AssistedInject constructor(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ChatItemDTO> {
        ...
        return try {
            val response = runRequestSafelyNotResult(
                ...
                action = { token ->
                    chatApi.getTrainerChatMessages(token, userId, cursor)
                }
            )
            val dtos: MutableList<ChatItemDTO> = mutableListOf()
            var lastDate = TrainerMessagePage.lastDate
            var newTime = ""
            response.objects.forEachIndexed { index, dto ->
                newTime = parseDateToDDMM(dto.time)
                dtos.add(dto)
                if (index == 0)
                    TrainerMessagePage.lastDate = newTime
